...
http://web.co.nz/~grantm/cpan/ , unpack it and run these commands:
...
If that doesn't work, you really only need the Simple.pm file, so grab it
from the above site and save it in the \site\lib\XML directory under your
Perl installation (typically C:\Perl).
...
'make test' to me at grantm@web.co.nz
